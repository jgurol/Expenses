...
serve(async (req) => {
  ...
  try {
    ...
    // Construct the password reset URL correctly
    const supabaseUrl = Deno.env.get('SUPABASE_URL');
    const resetUrl = `${supabaseUrl}/auth/v1/verify?token=${email_data.token_hash}&type=${email_data.email_action_type}&redirect_to=${encodeURIComponent(email_data.redirect_to)}`;
